...
### 4. Enable HTTPS (built-in)

The server now supports HTTPS directly via environment variables.

Quick local HTTPS (self-signed, autogenerated each run):
...
Persistent certificate/key HTTPS:
...
For full step-by-step certificate setup, see [HTTPS_SETUP.md](HTTPS_SETUP.md).
...
6. Authorized redirect URIs:
   - HTTP mode: `http://localhost:5000/auth/google/callback`
   - HTTPS mode: `https://localhost:5000/auth/google/callback`
...
3. Go to "OAuth2" tab
4. Add redirect:
   - HTTP mode: `http://localhost:5000/auth/discord/callback`
   - HTTPS mode: `https://localhost:5000/auth/discord/callback`
...
4. Add redirect URI:
   - HTTP mode: `http://localhost:5000/auth/microsoft/callback`
   - HTTPS mode: `https://localhost:5000/auth/microsoft/callback`
...
2. **Use HTTPS**:
   - Built-in TLS: set `HTTPS_ENABLED=True` and either:
     - use adhoc certs (no cert paths), or
     - set `SSL_CERT_FILE` + `SSL_KEY_FILE`
   - For production internet exposure, prefer nginx/Apache reverse proxy with managed certificates
